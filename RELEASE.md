...
Mastery Ledger has two release surfaces: the Python application and the `mastery-ledger/` Codex skill. The application is currently version `0.1.2`; compatibility with the skill is communicated through the versioned `doctor-v2` contract. The contract exposes application readiness only; source and course tooling belongs to the skill.
...
A tag matching `v*` starts the release workflow. The workflow requires the tag to equal the version in `pyproject.toml`, reruns tests, builds the wheel and source distribution, archives the matching Codex skill, builds and smoke-tests the portable Windows application, creates `SHA256SUMS.txt`, records GitHub artifact attestations, uploads immutable workflow artifacts, and creates the GitHub release. The in-app updater discovers the latest stable release through GitHub's release API and requires the exact `MasteryLedger-windows-x64-v<version>.zip` asset plus its GitHub SHA-256 digest.
...
5. Create and push the matching annotated version tag, such as `v0.1.2`.
