...
**Implementation status:** the `dashboard-v1` API and React Exam Ledger landing page now discover registered `course.yaml` or `study.yaml` manifests, ready exam definitions, question banks, source manifests, and review queues. Search, course filtering, the fixed-height exam register, recent-course summaries, the complete curve rail, responsive expansion, and a read-only exam detail sheet are implemented. Starting reviews, editing the curve from settings, and Focused Question delivery remain later slices.
