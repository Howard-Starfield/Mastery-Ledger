...
**Implementation status:** the application persists an active versioned curve and bounded profile history in SQLite while retaining compatible `review_intervals`. Each scheduled question records its curve ID, version, and interval sequence. `future_advancement` writes a pending curve without changing `next_due_at`, `new_questions_only` leaves existing curve assignments intact, and `recalculate_all` requires confirmation, recomputes pending dates from a recorded or safely inferred scheduling anchor, and synchronizes concept-level next-review dates. The React editor implements direct stage editing, human horizon labels, non-blocking warnings, reset, duplicate-as-new-profile, and the three policy choices.
